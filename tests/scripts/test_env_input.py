...
from scripts.env_input import (
    BASE_DIR,
    ENV_FILES,
    app,
    create_base_env_content,
...
def mock_env_files(temp_env_dir):
    ...
    with patch("scripts.env_input.ENV_FILES", mock_files):
        with patch("scripts.env_input.BASE_DIR", temp_env_dir):
            yield mock_files
...
class TestKeyGeneration:
    ...
    def test_generate_secure_keys_creates_unique_keys(self):
        ...
        for key in ["jwt_secret_dev", "jwt_secret_prod", "security_secret_dev", "security_secret_prod"]:
...
class TestCLICommands:
    ...
    def test_show_info_command_success(self, runner, mock_env_files):
    ...
    @patch("scripts.env_input.pbar_process")
    def test_env_setup_command_creates_files(self, mock_pbar, mock_keys, runner, mock_env_files):
...
class TestFailFastBehavior:
    ...
    def test_env_setup_fails_fast_on_exception(self, runner, mock_env_files):
        ...
        with patch("scripts.env_input.generate_secure_keys", side_effect=Exception("Test error")):
            result = runner.invoke(app, ["env-setup"])
            assert result.exit_code == 1
...
class TestDirectoryStructure:
    ...
    @patch("scripts.env_input.generate_secure_keys")
    @patch("scripts.env_input.pbar_process") 
    def test_env_setup_creates_secrets_directory(self, mock_pbar, mock_keys, runner, mock_env_files):
