"""module ini adalah orchestrator untuk startup."""


# urutan task
# 1-
class StartUpManager:
    """class ini adalah manager untuk startup.

    task 1: generation of envs file > task di delegate ke evnmanager/
    task 2: generation of files file > task di delegate ke datamanager/
    task 3: start up logic yg memungkinkan passing values ke terminal untuk startup aplikasi dengan typer choices
    semisal :
        - apakah mau run APP_ENV=PROD uvicorn src.main:app --host --port --workers
        - apakah mau run APP_ENV=DEV uvicorn src.main:app --host --port --reload
